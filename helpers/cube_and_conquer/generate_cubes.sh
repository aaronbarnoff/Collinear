...
Options:
  ...
  --zr march free variable elimination cutoff value
  --zl march cube number limit
  --zc march cadical conflict value (simplify for march)
  -h   help
EOF
}

options=$(getopt -o "hk:n:l:s:v:a:c:x:y:b:f:t:r:e:z:j:" \
                 -l "zr:,zl:,zc:" -- "$@")
eval set -- "$options"

k= n= l= s= v= a= c= x= y= b= f= t= r= e= z= j= zr= zl= zc=
...
: "${x:=0}" "${y:=0}" "${s:=1}" "${c:=0}" "${v:=1}" "${a:=0}" "${l:=0}" "${b:=2}" "${f:=1}" "${t:=0}" "${r:=0}" "${z:=0}" "${j:=0}" "${zr:=0}" "${zl:=0}" "${zc:=0}"


res_name="res_k${k}_n${n}_x${x}_y${y}_s${s}_c${c}_v${v}_a${a}_l${l}_b${b}_f${f}_r${r}_e${e:-none}_${run_id}"

python3 -u encode.py -k "$k" -n "$n" -l "$l" -a "$a" -v "$v" -c "$c" -s "$s" -x "$x" -y "$y" -b "$b" -t "$t" -f "$f" -r "$r" -p "$res_name" ${e:+-e "$e"} -j "$j" -z "1" --zr "$zr" --zl "$zl" --zc "$zc"
python3 -u generate_knf_cube_dimacs.py -f "$PWD/output/$res_name"

...
# now using a random seed

###### Points to run ###### (outdated as of 7.10.2026 now)
...
# n=280(139,140): ./run_loop_bounds.sh -x 139 -y 140 -b 2 -r 1
# (Writes/reads FAs in Collinear/fixed_assignments/fixed_assignments_n280_x139_y140_f0_j10.txt)

# n=x+y+1 
# CNF: f=0 (default)            
# KNF: f=1
# heuristic_off (k+0): j=0     
# heuristic_on (k+10): j=10 (default)  
...
Options:
  ...
  -j   line heuristic threshold value (default: 10, (k+10))
  -h   help
EOF
}

if ! options=$(getopt -o hx:y:f:b:r:m:j: -- "$@"); then
  usage
  exit 2
fi
eval set -- "$options"
...
case "$j" in *[^0-9]*|"") echo "j must be a non-negative integer"; exit 2 ;; esac

random_seed=$(shuf -i 1-1000000 -n 1)
...
echo "Started: $(date)"
echo "Initial parameters: x=$x y=$y f=$f b=$b seed=$random_seed ($bounds, $solve_type, $loop)"
if ((m>0)); then
echo "reading in fixed assignments"
fi
...
while :; do
  n=$(( x + y + 1 ))
    ...
    res_name="${bounds}_x${x}_y${y}_${solve_type}_j${j}_${run_id}"
    echo "Solving for ${bounds} bounds, point n=${n}:(${x},${y})"

    python3 -u encode.py -k 7 -n "$n" -x "$x" -y "$y" -l 0 -a 0 -v 1 -c 0 -s 1 -b 2 -t 0 -f "$f" -r $random_seed -p "$res_name" -j "$j" -w 0 --trim 0 --flip 0 --FA "$m"

    python3 -u solve.py  -k 7 -n "$n" -x "$x" -y "$y" -t 0 -f "$f" -r 0 -p "$res_name" -z 0 -w 0 -j "$j"
    res=$?
    ...
    if (( b == 0 )); then
        ...
        if (( res == 20 )); then
            ...
            echo "FAILURE:   ${bounds} n=${n}:(${x},${y}) ${solve_type}"
            break
        else

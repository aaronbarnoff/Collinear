...
run_point() {
  ...
  res_name="allpts/res_k${k}_n${n_cur}_x${x}_y${y}_s${s}_c${c}_v${v}_a${a}_l${l}_b${b}_f1_r${r}_${run_id}"
  python3 -u encode.py -k "$k" -n "$n_cur" -l "$l" -a "$a" -v "$v" -c "$c" -s "$s" -x "$x" -y "$y" -f "1" -b "$b" -t "$t" -r "$r" -p "$res_name"
  python3 -u solve.py  -k "$k" -n "$n_cur" -x "$x" -y "$y" -t "$t" -r "$r" -f "1" -p "$res_name"
